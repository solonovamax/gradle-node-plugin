package com.github.gradle.node.corepack.exec
...
abstract class CorepackExecRunner {
    @get:Inject
    abstract val providers: ProviderFactory

    fun executeCorepackCommand(
        ...
        val corepackDirProvider = variantComputer.computeCorepackDir(nodeExtension)
        val corepackBinDirProvider = variantComputer.computeCorepackBinDir(corepackDirProvider, nodeExtension.resolvedPlatform)
        val corepackExecProvider = variantComputer.computeCorepackExec(nodeExtension, corepackBinDirProvider)
        val additionalBinPathProvider =
            computeAdditionalBinPath(nodeExtension, nodeDirProvider, corepackBinDirProvider, variantComputer)
        val execConfiguration = ExecConfiguration(
            corepackExecProvider.get(),
    ...
    private fun computeAdditionalBinPath(
        ...
        corepackBinDirProvider: Provider<Directory>,
        variantComputer: VariantComputer,
        ...
        return zip(nodeBinDirProvider, npmBinDirProvider, corepackBinDirProvider).map { (nodeBinDir, npmBinDir, corepackBinDir) ->
            listOf(corepackBinDir, npmBinDir, nodeBinDir).map { file -> file.asFile.absolutePath }
        }
    }
}
