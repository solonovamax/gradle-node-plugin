...
class CorepackPnpmTaskTest extends AbstractTaskTest {
    def "exec pnpm task"() {
        given:
        nodeExtension.resolvedPlatform.set(PlatformHelperKt.parsePlatform("Linux", "x86_64", {}))

        def task = project.tasks.create('simple', CorepackPnpmTask)
        ...
        1 * execSpec.setExecutable({ it.contains('corepack') })
        1 * execSpec.setArgs(['pnpm', 'a', 'b'])
    }
    ...
    def "exec pnpm task (windows)"() {
        ...
        1 * execSpec.setIgnoreExitValue(true)
        1 * execSpec.setEnvironment({ it['a'] == '1' && containsPath(it) })
        1 * execSpec.setExecutable({ it.contains('corepack.cmd') })
        1 * execSpec.setArgs(['pnpm', 'a', 'b'])
    }
