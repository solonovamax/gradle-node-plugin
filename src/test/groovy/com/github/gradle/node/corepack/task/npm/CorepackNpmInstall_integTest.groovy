...
class CorepackNpmInstall_integTest extends AbstractIntegTest {
    def 'install packages with npm (#gv.version)'() {
        ...
        when:
        def result = build('corepackNpmInstall')
        ...
        when:
        result = build('corepackNpmInstall')
        ...
        result.task(":nodeSetup").outcome == TaskOutcome.SKIPPED
        result.task(":corepackSetup").outcome == TaskOutcome.UP_TO_DATE
        result.task(":corepackNpmInstall").outcome == TaskOutcome.UP_TO_DATE
...
"""
    ...
    def 'configure npm install to use the ci command through extension (#gv.version)'() {
        ...
        when:
        def result = buildAndFail('corepackNpmInstall')

        then:
        result.output.contains('can only install with an existing package-lock.json')
        result.task(':corepackNpmInstall').outcome == TaskOutcome.FAILED

        when:
        writeEmptyLockFile()
        result = buildTask('corepackNpmInstall')
    ...
    def 'verify npm install inputs/outputs (#gv.version)'() {
        ...
        writeBuild('''
            ...
            def installTask = tasks.named("corepackNpmInstall").get()
    ...
    def 'verify output configuration (#gv.version)'() {
        ...
        when:
        def result2 = build("corepackNpmInstall")
        ...
        def result3 = build("corepackNpmInstall")
        ...
        def result4 = build("corepackNpmInstall")

        then:
        // This time the build should not be up-to-date and the file should be reset
        result4.task(":corepackNpmInstall").outcome == TaskOutcome.SUCCESS
        ...
        def result5 = build("corepackNpmInstall")

        then:
        // This time the build should not be up-to-date and the file should be reset
        result5.task(":corepackNpmInstall").outcome == TaskOutcome.SUCCESS
